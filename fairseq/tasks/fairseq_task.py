...
def regularization(model, regularization_type):
    regu, counter = 0, 0
    for name, param in model.named_parameters():
        if "scores" in name:
            if regularization_type == "l1":
                ...
                ValueError(f"Don't know this regularization type: {regularization_type}.")
            counter += 1
    return regu / counter
...
class FairseqTask(object):
    ...
    def train_step(
        ...
        mask_regularization_type = "l1", mask_regularization_lambda=0
    ):
        """
        ...
        if ignore_grad:
            loss *= 0
        if mask_regularization_lambda > 0.: 
            reg = regularization(model, mask_regularization_type)
            loss += mask_regularization_lambda * reg

        with torch.autograd.profiler.record_function("backward"):
            optimizer.backward(loss)
            
        # NOTE: we always return the original loss, so that we can simply compare the loss in the training plot (if needed)
        if mask_regularization_lambda > 0.: 
            return loss - mask_regularization_lambda * reg, sample_size, logging_output
        else:
