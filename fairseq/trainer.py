...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            # print(load_on_all_ranks, self.data_parallel_rank == 0)
            # raise Exception("Stop here")
            ...
            #         print(name, param.data.view(-1)[0])
            # print('*' * 100)
                    

            try:
                # print(state["model"]["classification_heads.sentence_classification_head.out_proj.weight"].size())
                if "masked_roberta" in self.cfg.model._name:
                    raise ValueError("Hack Roberta Init")
                # print(self.cfg.model._name)
                # exit()
                ...
                #         # _POS = find_string(name, '.')
                #         # print(name[_POS:])
                #         # if name[]
                #         if name in state["model"].keys():
                #             print(100 * '*')
                #             print(f"This {name} loads weight from pre-trained model")
                #             # param.data = state["model"][name].to(param.device)
                #             print(param.data.cpu().norm(), state["model"][name].norm(), param.device)
                ...
                # )
                logger.warning(
                        f"We are using a very hack way to load data for Roberta model! {e}"
                )
                # print(state["model"].keys())
                # for name, module in self.get_model().named_modules():
                #     for p_n, p_p in module.named_parameters():
                #         print(name, p_n) 
                # for name, param in self.get_model().named_parameters():
                #     if name in state['model'].get(name):
                #         print(100 * '*')
                #         print(f"This {name} loads weight from pre-trained model")
                #         param.data = state["model"][name]
                #         print(param.data.cpu().norm(), state["model"][name].norm())
                #     # print(name)
                # exit()
                ...
                print(self.cfg.model.load_ckpt)
                for name, param in self.get_model().named_parameters():
                    # print(name, param.requires_grad, state["model"].get(name))
                        # _POS = find_string(name, '.')
                        # print(name[_POS:])
                        # if name[]
                    if name in state["model"].keys():
                        if (self.cfg.model.load_ckpt == 'all') or \
                           (self.cfg.model.load_ckpt == 'embed' and 'embed' in name):
                            print(100 * '*')
                            print(f"This {name} loads weight from pre-trained model")
                            dtype = param.dtype
                            param.data = state["model"][name].to(param.device).type( dtype )
                            print(param.data.cpu().norm(), state["model"][name].norm(), param.device)
                            # param.data = state["model"][name]
                # exit()
            # asdf
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output = self.task.train_step(
                        ...
                        mask_regularization_type=self.cfg.mask.mask_regularization_type,
                        mask_regularization_lambda=self.cfg.mask.mask_regularization_lambda,
                    )
                    del loss
